...
| **H3 Hybrid Keyframes** (`H3HybridKeyframes`) | Stills pinned at frame numbers of the whole run: one socket per image (a new one appears as you connect) and/or a whole batch in `image_batch`, one list of frame numbers such as `0, 15, 32, 64, 100` (sockets first, then the batch), negative numbers counted from the end. Hybrid Windows places each still into the window(s) that draw its frame, converts the number to that window's own index, fits it to the canvas, encodes it, and lists every placement in its report. Same conditioning rows as core's Add Guide, without a chain of guide nodes per window. See [keyframes](docs/keyframes.md). | FL2VA and R2V Keyframes variants |
